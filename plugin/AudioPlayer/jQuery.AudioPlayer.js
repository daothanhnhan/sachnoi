...
;(function(jQuery){
    ...
    var AudioPlayer = {
        ...
        ,events: function() {
            ...
            _this.elements.audioDom.oncanplay = function() {
                ...
                if (_this.elements.audioDom.readyState == 4) {
                    ...
                    _this.elements.totalTime.html(hr + ':' + min + ":" + (10 <= duration ? duration : "0" + duration));
        ...
        ,tuan: function() {
            var _this = this;
            // alert('tuan');
            _this.elements.audioDom.currentTime = _this.options.setTimeStar;
            // _this.elements.totalTime.html(_this.formatTime(_this.elements.audioDom.duration));
            // _this.elements.totalTime.html(100);
            // alert('tuan1');
        },ajax: function(giay) {
            var _this = this;
            
            var product_id = _this.options.product_id;
            var sub_id = _this.options.sub_id;
            const xhttp = new XMLHttpRequest();
              xhttp.onload = function() {
                // document.getElementById("demo").innerHTML = this.responseText;
                }
              xhttp.open("GET", "/functions/ajax/audio.php?product_id="+product_id+"&giay="+giay+"&sub_id="+sub_id, true);
              xhttp.send();
        }
    }
