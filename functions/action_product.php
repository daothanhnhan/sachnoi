...
public function getProductList_byMultiLevel_orderProductId_nghe_nhieu($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($this->nameColId_product != "" && $order_state != "") {
        ...
        $order = "order by dang_nghe $order_state ";
...
public function getProductList_byMultiLevel_orderProductId_search($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if (isset($_POST['q'])) {
        ...
        if (!empty($q)) {
            ...
            foreach ($q_arr as $item) {
                $item = vi_en1($item);
                $where .= " and friendly_url LIKE '%$item%'";
            }
            $where .= ")";
        }
        $s = vi_en1($q);
    ...
    if ($trang != "" && $limit != "") {
        ...
        $config = array(
            ...
            'link_full'=> $slug != '' ? $slug.'/{trang}' : '/'.$page.'/{trang}'.$cond,
